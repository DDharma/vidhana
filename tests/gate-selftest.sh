...
set -u
src=$(cd "$(dirname "$0")/.." && pwd); tmp=$(mktemp -d); cp -r "$src/.claude" "$tmp/"; mkdir -p "$tmp"/pipeline/{specs,designs,plans,reviews,qa,reports}
...
t "designer, nothing written → block"       designer 2
echo screens > pipeline/designs/T-001.md
t "designer, no DESIGN.md → block"          designer 2
echo '# Design System' > DESIGN.md; : > pipeline/designs/T-001.md
t "designer, DESIGN.md but empty item design → block" designer 2
echo screens > pipeline/designs/T-001.md
t "designer, item design + DESIGN.md → allow" designer 0
